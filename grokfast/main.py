...
def main(args):
    wandb.init(project="grokfast", config=args)  # Initialize wandb
    config = wandb.config
    ...
    model = Decoder(
        dim=128, num_layers=2, num_heads=4, num_tokens=config.p + 2, seq_len=5
    ).to(device)
    nparams = sum([p.numel() for p in model.parameters() if p.requires_grad])
    print(model)
    print(f'Total number of parameters: {nparams}')
    ...
    data = multiplication_mod_p_data(config.p, eq_token, op_token)
    ...
    steps_per_epoch = math.ceil(train_data.shape[1] / config.batch_size)
    ...
    for e in tqdm(range(int(config.budget) // steps_per_epoch)):
        ...
        for data, is_train in [(train_data, True), (valid_data, False)]:
            ...
            dl = torch.split(data, config.batch_size, dim=1)
            for input in dl:
                input = input.to(device)
                ...
                if is_train:
                    ...
                    trigger = i < 500 if config.two_stage else False

                    if config.filter == "none":
                        pass
                    elif config.filter == "ma":
                        grads = gradfilter_ma(model, grads=grads, window_size=config.window_size, lamb=config.lamb, trigger=trigger)
                    elif config.filter == "ema":
                        grads = gradfilter_ema(model, grads=grads, alpha=config.alpha, lamb=config.lamb)
                    else:
            ...
            if is_train:
                ...
                train_loss.append(total_loss / train_data.shape[-1])
                its.append(i)
                wandb.log({"train_acc": train_acc[-1], "train_loss": train_loss[-1], "step": i})
            else:
                val_acc.append(total_acc / valid_data.shape[-1])
                val_loss.append(total_loss / valid_data.shape[-1])
                wandb.log({"val_acc": val_acc[-1], "val_loss": val_loss[-1], "step": i})

        if config.save_weights:
            do_save = e <= 500 or (e > 500 and (e + 1) % 100 == 0) or e == int(config.budget) // steps_per_epoch - 1
        else:
            do_save = (e + 1) % 100 == 0
        if do_save:
            steps = torch.arange(len(train_acc)).numpy() * steps_per_epoch
            ...
            plt.xscale("log", base=10)
            plt.grid()
            plt.savefig(f"results/acc_{config.label}.png", dpi=150)
            plt.close()
            ...
            plt.xscale("log", base=10)
            plt.grid()
            plt.savefig(f"results/loss_{config.label}.png", dpi=150)
            plt.close()
            ...
            torch.save(results, f"results/res_{config.label}.pt")
            wandb.save(f"results/res_{config.label}.pt")

if __name__ == "__main__":
    # Example of how args might be parsed
    import argparse
