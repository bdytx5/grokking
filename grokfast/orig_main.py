...
def main(args):
    ...
    wandb.init(project="grokfast", config=args)
    config = wandb.config
    print(config)
    ...
    for e in tqdm(range(int(config.budget) // steps_per_epoch)):
        ...
        for data, is_train in [(train_data, True), (valid_data, False)]:
            ...
            wandb.log({
                'train_loss': train_loss[-1] if is_train else None,
                'train_acc': train_acc[-1] if is_train else None,
                'val_loss': val_loss[-1] if not is_train else None,
                'val_acc': val_acc[-1] if not is_train else None,
                'step': i
            })
